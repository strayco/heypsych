...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    logger.debug(
      `🔍 Search loaded: ${treatments.length} treatments, ${conditions.length} conditions, ${resources.length} resources`
    );
    ...
    const findMatchingSnippets = (item: any) => {
      ...
      for (const term of searchTerms) {
        ...
        if (!snippetAdded && description && description.toLowerCase().includes(term)) {
          ...
          const fullSnippet =
            (start > 0 ? "..." : "") + rawSnippet + (end < description.length ? "..." : "");
    ...
    const searchResults = [
      ...treatments.filter(matchesAllTerms).map((item) => {
      ...
      ...conditions.filter(matchesAllTerms).map((item) => {
        const snippets = findMatchingSnippets(item);
        return {
      ...
      ...resources.filter(matchesAllTerms).map((item) => {
        const snippets = findMatchingSnippets(item);
        return {
    ...
    const ranked = searchResults.sort((a, b) => {
      ...
      const aBrandMatch = aTreatment?.metadata?.brand_names?.some(
        (bn: string) =>
          bn.toLowerCase() === searchTerm || searchTerms.some((term) => bn.toLowerCase() === term)
      );
      const bBrandMatch = bTreatment?.metadata?.brand_names?.some(
        (bn: string) =>
          bn.toLowerCase() === searchTerm || searchTerms.some((term) => bn.toLowerCase() === term)
      );
