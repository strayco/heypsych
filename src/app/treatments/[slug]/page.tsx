          ...
          {Object.entries(sectionData).map(([severity, items]: [string, any]) => {
            ...
            return (
              <div key={severity} className={`rounded-lg border p-4 ${colorClass}`}>
                <h4 className="mb-2 font-semibold capitalize text-neutral-900">{severity.replace(/_/g, " ")}</h4>
        ...
        <div className="space-y-3">
          {Object.entries(sectionData).map(([key, value]: [string, any]) => {
            if (Array.isArray(value)) {
              return (
                <div key={key}>
                  <h4 className="mb-2 font-semibold capitalize text-neutral-900">{key.replace(/_/g, " ")}</h4>
                  <ul className="list-disc space-y-1 pl-6 text-neutral-800">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex items-start justify-between">
            <div className="space-y-3">
              ...
              {summary && <ParsedContent content={summary} className="max-w-2xl text-neutral-800" />}
              {description && (
                <div className="mt-4">
                  <Card>
