-- Simplify snippet extraction to match simplified search
-- Only extract snippets from the same fields we're searching (title, description, tags)
-- No more deep JSON content extraction
...
BEGIN
  ...
  WITH all_matches AS (
    SELECT
      ...
      -- Extract snippet from same fields as search: title + description + tags
      -- No more deep JSON content to keep it simple and direct
      ts_headline(
        'english',
...
COMMENT ON FUNCTION search_entities_grouped IS 'Simplified direct search - searches and extracts snippets from title, description, and tags only';
