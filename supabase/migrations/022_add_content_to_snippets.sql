-- Add content to snippet extraction so we can show where matches occur in article body
...
BEGIN
  ...
  WITH all_matches AS (
    SELECT
      ...
      -- Extract snippet from description, title, tags, and content
      -- Same fields we're searching, so snippet shows WHERE the match occurred
      ts_headline(
        'english',
...
COMMENT ON FUNCTION search_entities_grouped IS 'Direct search with content included - snippets show where match occurred';
