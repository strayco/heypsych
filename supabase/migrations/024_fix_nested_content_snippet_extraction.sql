-- Fix snippet extraction for nested content structures
-- Some resources store content at content->sections, others at content->content->sections
-- This migration handles both patterns using COALESCE
...
BEGIN
  ...
  WITH all_matches AS (
    SELECT
      ...
      -- Build searchable text from structured fields, checking both nesting levels
      ts_headline(
        'english',
        -- Description and title (always clean)
        COALESCE(e.description, '') || ' ' ||
        COALESCE(e.title, '') || ' ' ||
        -- Tags (check both paths: content->tags and content->content->tags)
        COALESCE(
        ...
        -- Summary field (check both paths)
        COALESCE(e.content->>'summary', e.content->'content'->>'summary', '') || ' ' ||
        -- For articles/resources: body sections as plain text (check both nesting levels)
        COALESCE(
        ...
        -- For conditions: treatment medications as plain text list (check both nesting levels)
        COALESCE(
        ...
        -- Real life examples (check both paths)
        COALESCE(
          (SELECT string_agg(value::text, ' ')
...
COMMENT ON FUNCTION search_entities_grouped IS 'Search with clean snippet extraction from structured content fields, handles both nesting levels';
