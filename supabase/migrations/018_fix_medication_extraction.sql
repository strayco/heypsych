-- Fix medication extraction for snippet generation
-- Medications are in treatment_approaches.medications as text array for conditions
...
BEGIN
  ...
  WITH all_matches AS (
    SELECT
      ...
      -- Build searchable text from same sources as search_vector
      ts_headline(
        'english',
        -- Concatenate all searchable fields
        COALESCE(e.description, '') || ' ' ||
        COALESCE(e.title, '') || ' ' ||
        -- Extract medications from treatment_approaches for conditions
        COALESCE(
          (SELECT string_agg(value::text, ' ')
           FROM jsonb_array_elements_text(e.content->'treatment_approaches'->'medications')),
          ''
        ) || ' ' ||
        -- Extract other searchable metadata
...
COMMENT ON FUNCTION search_entities_grouped IS 'Literal search with snippet extraction from description + medications + content fields';
