-- Add snippet extraction to search results for better UX
-- Returns a text snippet showing where the search term was found
...
BEGIN
  ...
  WITH all_matches AS (
    SELECT
      ...
      -- Search content first, then fall back to title+description
      ts_headline(
        'english',
        COALESCE(e.content::text, e.title || ' ' || COALESCE(e.description, '')),
        search_query,
...
COMMENT ON FUNCTION search_entities_grouped IS 'Simple literal search with snippet extraction - shows context where search term was found';
